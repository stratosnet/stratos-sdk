...
export class StratosSigningStargateClient extends SigningStargateClient {
  ...
  public async execEvm(
    ...
    const gasTipCap = ethers.BigNumber(payload.gasTipCap || '0'); // NOTE: Useless but keeped for a london sync
    const gasFeeCap = ethers.BigNumber(payload.gasFeeCap || minGasPrice.toString());
    const to = payload.to || '0x';
    const value = ethers.BigNumber(payload.value || 0);
