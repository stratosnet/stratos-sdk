...
const sendUserUploadSignRequest = async (
  ...
  if (!responseUploadSign) {
    const errorMsg = `There was an error during the upload sign. "response" from the request is empty , Details: (${JSON.stringify(
      callUploadSignResult,
    )})`;
    progressCb({
  ...
  if (uploadSignReturnValue !== '0') {
    const errorMsg = `There was an error during the upload sign. Non 0 code was returned, Details: ${JSON.stringify(
      responseUploadSign,
    )}`;
