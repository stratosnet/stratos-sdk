...
describe(`Stratos SDK integration (integration test)`, () => {
  ...
  describe('Prepay OZONE, SDS Remote file system', () => {
    ...
    describe('Prepay and OZONE', () => {
      ...
      it(
        'Check that account has ozone balance, assuming that for 0.1 STOS account should have at least 95 OZ',
          ...
          void Integration.getAccountOzoneBalance(0, receiverMnemonic).then(result => {
            expect(result).toBe(true);
            done();
          });
        },
    ...
    describe('Remote File System', () => {
      ...
      it(
        'Creates the remote file shared link from filehash',
        done => {
          void Integration.createSharedLinkForFile(fileReadName, randomPrefix, 0, receiverMnemonic).then(
            result => {
              expect(result).toBe(true);
              done();
            },
      ...
      it(
        'Check the get shared files list works and contains the shared file info.',
        done => {
          void Integration.getSharedFilesListAndCheckShare(
            fileReadName,
            randomPrefix,
            0,
            receiverMnemonic,
