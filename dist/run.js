...
const mainSdsPrepay = async (hdPathIndex, givenReceiverMnemonic = zeroUserMnemonic, memo) => {
    const keyPairZero = await stratos.crypto.hdVault.wallet.deriveKeyPairFromMnemonic(givenReceiverMnemonic, hdPathIndex);
    if (!keyPairZero) {
        return;
    }
...
const testTxHistory = async (hdPathIndex, page = 0, givenReceiverMnemonic = zeroUserMnemonic, givenAddress = '') => {
    const keyPairZero = await stratos.crypto.hdVault.wallet.deriveKeyPairFromMnemonic(givenReceiverMnemonic, hdPathIndex);
    if (!keyPairZero) {
        return;
    }
    ...
    try {
        const txList = await stratos.accounts.accountsApi.getAccountTrasactions(addressToUse, filterParameter, page, 5, 
...
const testGetSharedFileInfo = async (hdPathIndex, sharelink, // with or without sds://
givenReceiverMnemonic = zeroUserMnemonic) => {
    const keyPairZero = await stratos.crypto.hdVault.wallet.deriveKeyPairFromMnemonic(givenReceiverMnemonic, hdPathIndex);
    if (!keyPairZero) {
        return;
    }
    const sharedFileInfo = await stratos.sds.remoteFileSystem.remoteFileSystemApi.getSharedFileInfo(keyPairZero, sharelink);
