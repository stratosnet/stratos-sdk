...
const generateMasterKeySeed = async (phrase) => {
    ...
    const mnemonicChecked = new crypto_2.EnglishMnemonic(stringMnemonic);
    const seed = await crypto_2.Bip39.mnemonicToSeed(mnemonicChecked, (0, hdVault_1.bip39Password)((_a = Sdk_1.default.environment.keyPathParameters) === null || _a === void 0 ? void 0 : _a.bip39Password));
    return seed;
};
...
const signWithPrivateKey = async (signMessageString, privateKey) => {
    ...
    const signature = await crypto_2.Secp256k1.createSignature(encodedMessage, defaultPrivkey);
...
const verifySignature = async (signatureMessage, signature, publicKey) => {
    try {
        ...
        const restoredSignature = crypto_2.Secp256k1Signature.fromFixedLength(signatureData);
        const verifyResult = await crypto_2.Secp256k1.verifySignature(restoredSignature, encodedMessage, compressedPubkey);
        return verifyResult;
    }
    catch (err) {
        return Promise.reject(false);
