...
const Types = __importStar(require("./validatorsTypes"));
const getValidatorsBondedToDelegator = async (delegatorAddress) => {
    const vStatus = Types.ValidatorsNetworkStatus.All;
    const vListResult = await network_1.networkApi.getValidatorsBondedToDelegatorList(vStatus, delegatorAddress);
...
const getValidators = async (status = Types.ValidatorStatus.Bonded, page) => {
    ...
    const vListResult = await network_1.networkApi.getValidatorsList(vStatus, page);
    const { response } = vListResult;
    if (!response) {
        throw new Error('Could not fetch validators list');
    }
    const { validators: validatorResultList } = response;
    const vPoolResult = await network_1.networkApi.getStakingPool();
